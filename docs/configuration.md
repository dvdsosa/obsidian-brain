...
| `EMBEDDING_PROVIDER` | no | transformers | Embedding backend. 'transformers' (local, default), 'ollama' (requires a running Ollama server), or 'openai-compatible' (any server speaking POST /v1/embeddings — llama.cpp's llama-server, LM Studio, vLLM, text-embeddings-inference, or OpenAI itself). Aliases for the last one: llamacpp, llama.cpp, lmstudio, vllm, tei, openai. *Choices: transformers, ollama, openai-compatible, llamacpp, lmstudio, vllm, tei, openai* |
